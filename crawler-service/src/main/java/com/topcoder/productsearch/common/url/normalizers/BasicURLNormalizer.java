...
/**
 * Converts URL to Normal Form
 * 
 * Code adapted from Apache Nutch Normalization routines.
 * 
 * 
 * 
 */
...
/**
   * the logger instance
   */
  private static final Logger logger = LoggerFactory.getLogger(BasicURLNormalizer.class);
  ...
  private final Rule adjacentSlashRule;
    ...
    public BasicURLNormalizer() {
        try {
            ...
            relativePathRule.pattern = Pattern.compile(
                "(/[^/]*[^/.]{1}[^/]*/\\.\\./)");
            ...
            adjacentSlashRule.substitution = "/";
                ...
                logger.error("Error occured while compiling Pattern: ",e.getMessage());
    ...
    private String substituteUnnecessaryRelativePaths(String file) {
      ...
      while (oldLen != newLen) {
        ...
        Matcher relativePathMatcher = relativePathRule.pattern.matcher(fileWorkCopy);
        fileWorkCopy = relativePathMatcher.replaceFirst(relativePathRule.substitution);
  
        // remove leading "/../"
        Matcher leadingRelativeMatcher = leadingRelativePathRule.pattern.matcher(fileWorkCopy);
        fileWorkCopy = leadingRelativeMatcher.replaceFirst(leadingRelativePathRule.substitution);

  
        // remove unnecessary "/./"
        Matcher currentPathMatcher = currentPathRule.pattern.matcher(fileWorkCopy);
        fileWorkCopy = currentPathMatcher.replaceFirst(currentPathRule.substitution);

        // collapse adjacent slashes with "/"
        Matcher adjacentSlashMatcher = adjacentSlashRule.pattern.matcher(fileWorkCopy);
        fileWorkCopy = adjacentSlashMatcher.replaceFirst(adjacentSlashRule.substitution);
